...
class CIWS:
    ...
    def adjust_tracking_factor(self, jamming_level):
        ...
        if self.radar_local:
            tracking *= (1 - jamming_level * 0.2)  # Réduction moindre pour radar local
        else:
            tracking *= (1 - jamming_level * 0.4)  # Réduction plus forte pour radar déporté
        if self.eo_sensor:
            tracking = min(self.base_tracking_factor, tracking + (self.base_tracking_factor - tracking) * 0.7 * (
                        1 - jamming_level))  # EO plus résistant
        return max(0.1, tracking)  # Minimum pour éviter un suivi nul
    ...
    def simulate_intercept(self, time, missile_distance, missile, mode, dt, jamming_level=0.2):
        if self.min_range <= missile_distance <= self.max_range:
            ...
            y_pred = y_curr * (1 - tracking_factor if mode != 1 else 0)  # Pas d'erreur en Y en vol direct
            z_pred = z_curr * (1 - tracking_factor if mode != 1 else 0)  # Pas d'erreur en Z en vol direct
            ...
            dispersion_area = np.pi * radius ** 2 * (1 - tracking_factor)  # Aire ajustée par précision
            shot_density = shots_fired / max(dispersion_area, missile.surface)  # Éviter division par trop grand

            error_distance = np.sqrt((x_pred - x_real) ** 2 + (y_pred - y_real) ** 2 + (z_pred - z_real) ** 2)
            hit_prob = norm.cdf(radius, loc=error_distance,
                                scale=radius / 4) if mode != 1 else 0.9  # Prob élevée en vol direct
            expected_hits = shot_density * missile.surface * hit_prob * 3  # Facteur pour réalisme
...
exocet = Missile(
    name="Exocet MM40", speed=300, surface=2.0, range=5000,  # Réduit pour plus de temps d'engagement
    maneuver_g=5, zigzag_start=1000, zigzag_period=1000,  # Ajusté pour réalisme
